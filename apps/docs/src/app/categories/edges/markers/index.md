...
- **Edges**: Specify `start` and `end` markers for corresponding parts of the edge. The built-in markers are two
  `type`s: `arrow` and `arrow-closed`. `width`, `height`, `orient` and `markerUnits` are optional. Any other shape
...
arrowheads automatically. Bind `ctx.markerStart()` / `ctx.markerEnd()` in your edge template. For other stroke
widths, style the `vflow-marker` and `vflow-marker--<type>` classes of the marker elements.
...
`markerUnits` the marker asks for and the stroke of the edge, so a declared shape is used exactly like an arrow:
...
The shape draws in the viewBox `-10 -10 20 20` with its tip at `x = 0`, the point that touches the handle, and its
body towards negative `x`. `inset` is where the path ends, in marker units before the tip. Put it one unit inside
the back of the shape, under its stroke: the diamond above reaches `x = -10`, so its inset is `9`. The line then
neither leaves a gap at the back nor runs through an open circle or diamond; for a bar the inset is on the bar.
Set `fill="none"` or `fill="context-stroke"` on the shape yourself; stroke properties inherit from the
marker element, and your CSS can override them through the `vflow-marker` and `vflow-marker--<type>` classes. A
type without a declared shape renders an empty marker, and development mode warns about it.
