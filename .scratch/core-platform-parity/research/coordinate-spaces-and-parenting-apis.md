...
5. Document the canonical immutable recipe as `clientToFlowPosition` → `getNodesAtPoint` → application filter → `reparentNodes`; the helper already preserves the flow-space position while converting the stored node-space position. For a newly created node, the selected candidate's `nodeSpacePoint` can be stored directly. This is comparable to React Flow's primitives but avoids forcing every Angular consumer to rebuild containment from internal geometry.
