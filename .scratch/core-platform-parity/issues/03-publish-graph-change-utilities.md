...
- Publish `addNodes`, `removeNodes`, `reparentNodes`, `addEdges`, `removeEdges`, and `reconnectEdges` from one pure `utils/graph-operations` module. Helpers accept readonly inputs, apply each input left-to-right, and never maintain a store or write supplied signals.
...
- Append additions, retain survivor order, and replace reparented or reconnected entities in their existing slots. Preserve every unaffected entity and signal reference; return original collection references for a full no-op.
...
- Preserve a reparented node's flow-space position by replacing only its object and `point`/`parentId` signals. Moving a parent carries descendants without rewriting them.
...
- Removing a node cascades to its descendants and all incident edges by default. Issue 03 owns these pure mechanics; issue 06 owns request, veto/replacement, and reporting lifecycle.
...
- Additions append in input order. Survivors retain their order, object references, and signal references. Reparented nodes and reconnected edges replace the addressed entity in its existing array slot. Reparenting creates new `point` and `parentId` signals but reuses the node's other fields and signals; reconnection changes only the connection fields. A fully no-op helper call returns the original collection references.
...
- `removeNodes` returns next `nodes` and `edges` plus the exact `removedNodes` and `removedEdges` references so issue 06 can expose a deletion proposal without repeating cascade traversal.
...
- Verification: Prettier passed; all 96 library tests passed in Chrome Headless; library and demo lint passed; library and demo/docs builds passed.
