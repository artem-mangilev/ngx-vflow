...
The public entity model has isolated flags such as node `draggable`, edge `reconnectable`, and global `entitiesSelectable`, but no consistent selection, deletion, focus, or handle-connection policy. Consumers cannot declaratively make one node non-selectable, one edge non-deletable, or one handle unavailable as a connection start without filtering interaction events themselves.
...
- Add optional `WritableSignal<boolean>` fields named `selectable`, `deletable`, and `focusable` to the public node and edge models.
...
- Resolve `deletable` from the local entity field with a library default of `true`. There is no global deletion default.
- Add `enabled?: boolean` to `SelectionBoxSettings`, defaulting to `true`. This controls whether the selection-box gesture can start and is independent of node and edge selection eligibility.
- Remove `entitiesSelectable` in the major release. Document migration to `nodesSelectable`, `edgesSelectable`, and `selectionBox.enabled` rather than keeping an overlapping compatibility input.
...
Factories must preserve inheritance. `createNode()` and `createEdge()` must not materialize inherited selection, deletion, or focus defaults as entity overrides. Their materialized return types must therefore keep the new capability fields optional instead of relying on `Required<Node>` or `Required<Edge>` for those fields.
...
- Selection-box hit testing must exclude nodes and edges that resolve to non-selectable, and final selection applies that eligible preselection. The gesture itself is controlled only by `selectionBox.enabled`; no special policy-change recheck is required at release.
...
- Centralize precedence and resolved read-only signals in one internal capability-policy module used by node and edge models. Interaction paths must consume resolved state rather than reproduce fallback logic.
- Apply selection and handle policies to all pointer and selection-box paths that exist in this issue.
- Define and resolve `deletable` for issue 06, but leave deletion requests and structural cascade behavior to that issue.
...
- Nodes and edges independently opt out of future library-originated deletion through a local `deletable` override.
...
- Grilling resolved the public contract before implementation: use flat local entity flags and flat global selection/focus inputs, keep deletion local-only, separate selection-box gesture enablement from entity eligibility, omit visibility, and expose resolved policy only to handle templates.
