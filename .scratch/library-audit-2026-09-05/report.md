...
> Ниже сохранены результаты первоначального аудита. Исправления внесены по этому отчёту; текущие проверки находятся в `projects/ngx-vflow-lib/src/lib/vflow/audit-regressions.spec.ts`. `run.sh` теперь запускает их напрямую, без временного копирования. Проверка удалённого кэша заменена проверкой поведения HTML label.
